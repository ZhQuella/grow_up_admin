<template>
  <div class="p-[10px]">
    ...
    <g-dialog v-model="dialogConfig.visible" width="600px" :title="dialogConfig.title">
      <component :is="dialogConfig.conmponetName" :row="dialogConfig.data" />
    </g-dialog>
  </div>
</template>
...
const { menuTypeDicts, stateList } = useDict();

const { tableList, tableColumns } = useTableOption({
  page,
  size,
  menuTypeDicts,
  stateList
...
const { dialogConfig, buttonGroup, optionGroup } = useTableFunc();
</script>
