  ...
  <ElCard>
    <div class="p-[40px]">
      <div class="h-[100px] w-[100px] m-[auto] mb-[10px]">
        <el-icon class="w-full h-full flex justify-center" size="90px" color="#999">
          <Unlocked />
        </el-icon>
      </div>
      <ElForm @submit.native.prevent>
        <ElFormItem>
          <ElInput
            :placeholder="i18n.t('SYSTEM_CONTENT.UNLOCK')"
            class="w-[300px]"
            type="password"
            v-model="password"
            show-password
      ...
      <div class="flex flex-col">
        <div>
          <ElButton type="primary" class="block w-full" size="large" @click="onUnLockScreen">{{
            i18n.t("SYSTEM_CONTENT.BTN_UNLOCK")
          }}</ElButton>
        </div>
        <div class="pt-[10px]">
          <ElButton text class="block w-full" size="large" @click="onLoginOut">{{
            i18n.t("APP_SEARCH.LOG_OUT")
          }}</ElButton>
        </div>
      </div>
    </div>
  ...
  try {
    ...
    if (lockScreenStore.getCrackingTimes) {
      ElNotification({
        ...
        message: `${i18n.t("UNLOCK_TIP.BEFORE_CONTENT")}${lockScreenStore.getCrackingTimes}${i18n.t(
          "UNLOCK_TIP.AFTER_CONTENT"
        )}`,
        type: "warning",
        showClose: true
      });
