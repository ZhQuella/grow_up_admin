...
<template>
        ...
        <div class="flex flex-col h-full">
          <div class="flex justify-between grow-0 shrink-0 h-[40px] px-[10px]">
            <div class="flex-1 w-[1px]">
              <ElTabs v-model="tabsActive" @tab-change="onTabChange" @tab-remove="onTabsRemove">
                <ElTabPane
            ...
            <div class="py-[5px]">
              <ElTooltip effect="dark" content="Import Map" placement="top">
                <ElButton text type="primary" size="small" @click="onShowImportMap">
                  <ElIcon size="16">
                    <WorkspaceImport />
                  </ElIcon>
                </ElButton>
              </ElTooltip>
              <ElTooltip effect="dark" content="新增" placement="top">
              ...
              <ElTooltip effect="dark" content="运行" placement="top">
                <ElButton
                  text
                  type="primary"
                  size="small"
  ...
  <ElDialog
    ...
    title="文件名称"
  >
    <FileName @close="onClose" @submit="onSubmit" />
  </ElDialog>

  <ElDrawer v-model="drawerRef" title="设置导入" :modal="false" :destroy-on-close="true">
    <ImportMap @close="drawerRef = false" @submit="onImportSubmit" ref="importMapRef" />
  </ElDrawer>
</template>
...
import { GSplitPane } from "@grow-rock/grow-component";
...
const { treeData } = initView();
const { tabsActive, tabsOptions, onTabChange, onTabsRemove } = useTabsOption({
  codeOptions,
  options,
  codemirrorRef
});
const { onSubmit, onClose, onHandleTabsAdd, dialogVisible } = useEvent({
  codeOptions,
  tabsActive,
  codemirrorRef
});
const { drawerRef, onShowImportMap, onImportSubmit, importMapRef, catchImportRef } = useDrawer();
