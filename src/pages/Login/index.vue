  ...
  <div class="h-full bg-BG_COLOR overflow-hidden">
    <div class="-enter-y w-full h-[60px] box-border fixed z-20 flex px-[20px] justify-between"> 
      <div class="px-[5px] pt-[10px] cursor-pointer">
        <GLogo isOpacity @click="onGoToIndex"/>
      </div>
      <div class="flex">
        <div class="pt-[10px]">
    ...
    <div class="flex">
      <div class="h-[100vh] w-[400px] bg-BG_COLOR2 flex items-center justify-center overflow-hidden xl:flex sm:hidden">
        <div>
          <div class="px-[10px]">
            <h3 class="text-[30px] pl-[32px] pb-[10px] text-TEXT_LIGHT1 -enter-x">{{ $t("LOIN_STATE.STATE_1") }}</h3>
            <h3 class="text-[30px] pl-[64px] pb-[10px] text-TEXT_LIGHT1 -enter-x">{{ $t("LOIN_STATE.STATE_2") }}</h3>
            <h3 class="text-[30px] pl-[98px] text-TEXT_LIGHT1 -enter-x">{{ $t("LOIN_STATE.STATE_3") }}</h3>
          </div>
      ...
      <div class="h-[100vh] flex-1 relative">
        <div class="xl:w-[450px] md:w-[700px] sm:w-full absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] rounded-[4px] overflow-hidden border-solid border-[1px] pb-[40px] md:bg-BG_COLOR2 sm:bg-transparent md:border-BORDER_COLOR sm:border-transparent">
            ...
            <div v-if="formType === 'login'">
              ...
              <el-row :gutter="20">
                <el-col :span="12" class="-enter-x">
                  <el-button class="w-full" @click="onChangeFormType('mobilePhone')">{{ $t("LOGIN_WORD.NUMBER_LOGIN") }}</el-button>
                </el-col>
                <el-col :span="12" class="enter-x">
                  <el-button class="w-full" @click="onChangeFormType('qrCodeLogin')">{{ $t("LOGIN_WORD.SCAN_CODE_LOGIN") }}</el-button>
                </el-col>
              </el-row>
            </div>
            <!--忘记密码-->
