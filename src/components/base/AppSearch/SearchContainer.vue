<template>
  <div class="min-h-[100px]">
    <div v-if="state.filterList.length">
      <router-link v-for="(item) of state.filterList"
                  :key="item.label"
                  :to="{
                    path: `/home/${item.path}`
                  }"
                  @click="onSelect"
                  class="block mb-[5px] text-[14px] leading-[40px] p-[5px] overflow-hidden border-solid border-[1px] rounded-[4px] border-gray-300 text-gray-500 hover:bg-sky-700 hover:text-gray-300 transition-all">{{ item.label }}</router-link>
    </div>
    <p class="leading-[100px] text-center text-[12px] text-gray-400"
      v-else>{{ $t("APP_SEARCH.NOT_SEARCH_DATA") }}</p>
  </div>
</template>
